...
namespace GrandLarceny
{
	...
	public class LightCone : MovingObject
	{
		...
		private Vector2[] getTrianglePointsOffset()
		{
			...
			t_ret[1] = new Vector2((float)(m_length * Math.Cos(m_rotate) + (m_width / 3) * Math.Cos(1.5 * Math.PI + m_rotate)), (float)(m_length * Math.Sin(m_rotate) + (m_width / 2) * Math.Sin(1.5 * Math.PI + m_rotate)));
			t_ret[2] = new Vector2((float)(m_length * Math.Cos(m_rotate) + (m_width / 3) * Math.Cos(0.5 * Math.PI + m_rotate)), (float)(m_length * Math.Sin(m_rotate) + (m_width / 2) * Math.Sin(0.5 * Math.PI + m_rotate)));
